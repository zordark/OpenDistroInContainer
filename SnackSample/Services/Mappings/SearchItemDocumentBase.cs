...
namespace SnackSample.Services.Mappings
{
    public class SearchItemDocumentBase
    {
        ...
        internal virtual void PostProcess(IReadOnlyDictionary<string, IReadOnlyCollection<string>> highlights)
        {
            if (highlights?.Any(h => h.Key == nameof(Summary)) == true)
            {
                Summary = string.Join("...", highlights.First(p => p.Key == nameof(Summary)).Value);
            }
        }
    }
}
